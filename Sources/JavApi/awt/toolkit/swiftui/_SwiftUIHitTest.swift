...
/// CGPoint-convenience wrapper around `_SwingHitTest` for SwiftUI/CoreGraphics callers.
///
/// Always delegates to `_SwingHitTest` (which handles both AWT and Swing containers)
/// rather than `_AWTHitTest` directly.
@MainActor
enum _SwiftUIHitTest {
  ...
  static func find(at point: CGPoint, in root: java.awt.Component) -> java.awt.Component? {
    _SwingHitTest.find(x: Int(point.x), y: Int(point.y), in: root)
  }
  ...
  static func findWithLocal(at point: CGPoint, in root: java.awt.Component) -> (java.awt.Component, Int, Int)? {
    _SwingHitTest.findWithLocal(x: Int(point.x), y: Int(point.y), in: root)
  }
}
#endif
