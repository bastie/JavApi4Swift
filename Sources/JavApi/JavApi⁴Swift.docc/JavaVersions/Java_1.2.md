...
1.2     | ✔️          | ✔️       | open class    | LinkedList              | `LinkedList.swift`; implements `List` + `Queue` + `Deque` (since Java 1.6); bidirectional `ListIterator`; `LinkedListDescendingIterator` (tail→head); `LinkedList+Swiftify.swift`: `init(from:[E])`, `toSwiftArray()`, subscript, `Array.toJavaLinkedList()`
