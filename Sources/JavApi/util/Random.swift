...
extension java.util {
  ...
  open class Random : Hashable {
    ...
    public init() {
      ...
      setSeed(System.currentTimeMillis() + Int64(hasher.finalize()))// hashCode());
    }
     ...
     * initial state.
     *
     * - Parameter seed: the seed that will determine the initial state of this random number generator.
     */
    public init (_ seed : any FixedWidthInteger) {
      setSeed(Int64(seed))
    }
     ...
     *
     * - Parameter bits: number of bits of the returned value.
     * - Returns: a pseudo-random generated int number.
     */
    public func next(_ bits : Int) -> Int {
      seed = (seed &* java.util.Random.multiplier + Int64(0xb)) & ((Int64(1) << 48) - 1);
      return Int (seed >>> Int64(48 - bits));
    }
     ...
     *
     * - Parameter buf: non-null array to contain the new random {@code byte}s.
     */
    public func nextBytes(_ buf : inout [byte]) {
      var rand = 0
      var count = 0
      var loop = 0
      while (count < buf.length) {
        ...
        buf[count] = UInt8(truncatingIfNeeded: rand)
        count += 1
        rand >>= 8;
      }
    }
     ...
     *
     * - Returns: a random {@code double} in the range [0.0 - 1.0)
     */
    public func nextDouble() -> Double {
      let dividend = ((Int64) (next(26) << 27)) + Int64(next(27))
      let divisor = Double(Int64(1) << 53)
     ...
     * inclusively and 1.0 exclusively.
     *
     * - Returns: float a random {@code float} number between [0.0 and 1.0)
     */
    ...
    public func nextGaussian() -> Double{
      ...
      let norm : Double = Math.sqrt(-2 * Math.log(s) / s);
     ...
     *
     * - Returns: a uniformly distributed {@code int} value.
     */
     ...
     *
     * - Parameter n: the exclusive upper border of the range [0 - n).
     * - Returns: a random {@code int}.
     */
     ...
     *
     * - Parameter seed: the seed that alters the state of the random number generator.
     */
